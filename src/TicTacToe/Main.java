// Learn more on abstract actions
package TicTacToe;
...
public class Main extends JFrame implements ActionListener, ItemListener, Runnable {
	...
	public void setIndex(){
		...
		int[] numberPressed = {36, 38, 33, 37, 12, 39, 35, 40, 34};
	
				for(int a = 0; a < 9; a++){
					System.out.println(firstButtonPressed);
					if((pressed2 == lettersPressed[a] || pressed2 == numberPressed[a]) && point[a] == 0){
						playersTurn2 = false;
					if(player){
						graphics.addSquare(a + 1, ColorSelecterSquare.colorSquare);
						System.out.println("This prints twice");
					}else{
	...
	public void computerPlayer(){
		...
		//NORMAL DIFFIUCLTY
			...
			if(!positionFound){
				...
				/*
				if(point[1] == 0 && !positionFound){
					point[1] = 2;
					graphics.addCircle(2, ColorSelectorCircle.colorCircle);
		...
		//HARD DIFFICULTY
			...
			if(!positionFound){
				...
				if(!positionFound){
					...
					if(point[centerConstants[i]] == 0 && point[centerConstants[i+1]] == 0 && point[centerNumbers[i]] == number){
						activeSpaces[6] = centerConstants[i];
						activeSpaces[7] = centerConstants[i+1];
						activeSpacesTrueFalse = true;
					}
					
					if(point[centerHorizontalandVerticalConstants[i]] == 0 && point[centerHorizontalandVerticalConstants[i+1]] == 0 && point[centerNumbers[i]] == number){
					...
					if(point[horizontalConstants[i]] == 0 && point[horizontalConstants[i + 1]] == 0 && point[cornerNumbers[i]] == number){
						activeSpaces[0] = horizontalConstants[i];
						activeSpaces[1] = horizontalConstants[i + 1];
						activeSpacesTrueFalse = true;
					}
					
					if(point[diagonalConstants[i]] == 0 && point[diagonalConstants[i + 1]] == 0 && point[cornerNumbers[i]] == number){
						activeSpaces[2] = diagonalConstants[i];
						activeSpaces[3] = diagonalConstants[i + 1];
						activeSpacesTrueFalse = true;
					}
					
					if(point[verticalConstants[i]] == 0 && point[verticalConstants[i + 1]] == 0 && point[cornerNumbers[i]] == number){
					...
					if(activeSpacesTrueFalse == true){
						...
						while(choosingActiveSpaces){
							...
							System.out.println("Test Number: " + test2);
	...
	public void normalAndHardDifficultyMovement(){
		...
		for(int a = 1; a < 3; a++){
			for(int i = 0; i < 9; i++){
				...
				if(point[i] == a && point[horizontalConstants[i]] == a && i != 1 && i != 4 && i != 7 && point[horizontalShapePositions[i] - 1] == 0){
		...
		//vertical check
			...
			for(int a = 1; a < 3; a++){
				for(int i = 0; i < 9; i++){
					...
					if(point[i] == a && point[verticalConstants[i]] == a && i != 3 && i != 4 && i != 5 && point[verticalShapePositions[i] - 1] == 0){
						if((player && a == 1) || (!player && a == 2)){
							blockMove = true;
							iValueBlock = i;
		...
		int diagonalCornersPosition = 5;
			...
			for(int a = 1; a < 3; a++){
				for(int i = 0; i < 9; i += 2){
					if(point[i] == a && point[diagonalConstant] == a && i != 4 && point[diagonalShapePosition[i] - 1] == 0){
							if((player && a == 1) || (!player && a == 2)){
								blockMove = true;
								iValueBlock = i;
							...
							positionFound = true;
					}else if((i == 0 || i == 2) && point[i] == a && point[diagonalCorners[i]] == a && point[diagonalCornersPosition - 1] == 0){
						positionFound = true;
		...
		if(positionFound){
			...
			if(winMove){
				...
				if(positionBlock.equals("horizontal")){
					...
					if(player){
						graphics.addCircle(horizontalCornersShapePosition[iValueBlock], ColorSelectorCircle.colorCircle);
						point[horizontalCornersShapePosition[iValueBlock] - 1] = 2;
					}else{
						graphics.addSquare(horizontalCornersShapePosition[iValueBlock], ColorSelecterSquare.colorSquare);
	...
	public void keyBindings(boolean test){
		...
		String[] keyStroke = {"Q", "W", "E", "A", "S", "D", "Z", "X", "C", "HOME", "UP", "PAGE_UP", "LEFT", "CLEAR", "RIGHT", "END", "DOWN", "PAGE_DOWN"};
		int[] keyCode = {81, 87, 69, 65, 83, 68, 90, 88, 67, 36, 38, 33, 37, 12, 39, 35, 40, 34};
		if(test){
